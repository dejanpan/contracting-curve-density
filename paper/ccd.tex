...
\item \textbf{Initialization}: Given an input image $\mathbf{I}$ as training data, we first choose an initial
  ...
  initial values for the means and covariances ($P(\mathbf{\Phi})$). In addition, for most practical
...
\item \textbf{Convergence}: Check for the convergence of the 
  log posterior probability function $\log{P(\mathbf{\Phi}) P(\mathbf{I}|\mathbf{\Phi})}$. 
