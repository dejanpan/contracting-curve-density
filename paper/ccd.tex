...
vison because of its several outstanding features. \todos{Which ones??}
...
likelihood method was proven\todos{reference} not to work here, we have to use an alternative
...
curve-fitting problem as a Gaussian logistic \todos{Why is this not called classification?: regression} problem known in the
field of pattern recognition. Secondly, a quadratic or
a cubic B-spline curve is used to model the parametric curve
to avoid the Runge phenomenon~\todos{ref} without increasing the degree of the
...
optimization methods such as Conjugate Gradient Method\todos{ref}, 
Newton's method\todos{ref}, Gaussian-Newton\todos{ref} and Levenberg-Marquardt
...
\item \textbf{Convergence}: Check for the convergence of either
  the parameters or \todos{Which log cost function? log cost function.} 
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.49\linewidth} 
    \centering 
    \subfloat[iteration 20]{\includegraphics[width=1.5in]{experiments/tshirt/19.png}}
  \end{minipage} 
  \caption[Manual Initialization]{}
  \label{fig:book}
\end{figure}
