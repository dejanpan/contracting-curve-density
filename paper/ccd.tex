...
vison because of its several outstanding features. \todo{Which ones??}
...
likelihood method was proven\todo{reference} not to work here, we have to use an alternative
...
curve-fitting problem as a Gaussian logistic \todo{Why is this not called classification: regression} problem known in the
field of pattern recognition. Secondly, a quadratic or
a cubic B-spline curve is used to model the parametric curve
to avoid the Runge phenomenon~\todo{ref} without increasing the degree of the
...
\todo{Describe what CCD tracker is}\\
...
\todo{Below text shall probably go in Introduction section}
...
optimization methods such as Conjugate Gradient Method\todo{ref}, 
Newton's method\todo{ref}, Gaussian-Newton\todo{ref} and Levenberg-Marquardt
...
\item \textbf{Convergence}: Check for the convergence of either
  the parameters or \todo{Which log cost function? log cost function.} 
