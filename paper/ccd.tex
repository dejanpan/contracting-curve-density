...
the distribution of shapes. In this paper, for simplicity reasons, we 
consider the Gaussian distribution, which is commonly used in computer
vison because of its several outstanding features. \todos{Which
  ones??}\dones{Firstly, it does genuinely occur; Secondly,  the
  normal distribution can be manipulated algebraically much more
  easily than alternatives even when it just a rough approximation.}
...
curve-fitting problem as a Gaussian logistic \todos{Why is this not
  called classification?: regression} \dones{For the two-class
  classification problem, the posterior probability
of class $C_1$ can be written as a \textbf{CONTINUOUS} logistic sigmoid acting
on a linear function of $x$ (equivalent to \textit{fuzzy assignment}),
the main difference between regression and classification is that
whether desired output consists of one or more continuous
variables. In logistic regression problem, the considered variable is
the logistic function, which is a continous one.} problem known in the
field of pattern recognition. Secondly, a quadratic or
a cubic B-spline curve is used to model the parametric curve
to avoid the Runge phenomenon~\cite{wikibspline} without increasing the degree of the
...
\todod{Describe what CCD tracker is}\\
...
\item \textbf{Convergence}: Check for the convergence of either
  the parameters or \todos{Which log cost function? log cost
    function.} \dones{It should be: log posterior probability
    $\log{P(\Phi) P(I|\Phi)}$} 
...
As depicted in \todo{Fig.~\ref{fig:pointcloud}}, the PR2 robot simultaneously takes a three-dimensional scan and captures
...
\hline
experiments & fail rate& run time & iteration & tolerance\\
\hline
manual & 5\% & 3.35 & 26 & 0.001\\
\hline
sift & 9\% & 3.52 & 5 & 0.001\\
\hline
model & 4\% & 10.15 & 28 & 0.001\\
\hline
\end{tabular}}
...
& $\kappa$ & $c$ & $h$ & $\Delta_h$ &samples points & degree &dimension\\
\hline
