...
% row 1
 &
\node [block] (init) {initialize model curves: $\mathbf{\Phi}$}; &
\node [block] (rawdata) {input image: $\mathbf{I}$};
\\
% row 2
\node [block] (refine) {refine parameters}; & \node [block] (identify) {identify prior probability: $p(\mathbf{\Phi})$}; 
& \node [block] (evaluate) {evaluate local statistics: $p(\mathbf{I}|\mathbf{\Phi})$};\\
% row 3
&
\node [block] (map) {MAP estimate: $p(\mathbf{\Phi}|\mathbf{I})$}; &  \\
% row 4
& \node [decision] (decide) {test of convergence}; & \node [cloud] (irls) {IRLS};\\
% row 5
& \node [block] (stop) {output parameters: $\mathbf{\Sigma}$, $\mathbf{\Phi}$ }; & \\
};
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=\columnwidth]{cont.jpg}
...
On the other hand we should avoid collecting statistics from too few pixels, because
