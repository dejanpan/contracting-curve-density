...
speed up computation. In our implementation, the ideal case is that
...
\item \textbf{Noise reduction}. Many linear and non-linear algorithms
  ...
  Bi-linear interpolation and so on.
...
initialization of model contour. For the moment, we only consider 2-D
affine shape-space that consisting of 6 degrees of freedom. % It is followed by the process of
...
(add image). The B-spline curve is defined in (eq.~\ref{eq:paramcurve}) and is
...
Hence, there is a trade-off between computational expenses and
accuracy. We recommend to compute the circumference of a given object before
determining the value of $N_C$ and sample more points near spinodals and corners to
capture key features of the object.

The resulting parametric curve is continuous and
differentiable, we can compute the normal $\mathbf{n} = \{n_0, \ldots,
n_{k}\}$ and tangent vector $\mathbf{t} = \{t_0, \ldots, t_{k}\}$
conveniently, and this is illustrated in Section~\ref{sec:pbc}. 
...
model-based approach is that we can restrict a problem in a ROI, and is
...
curve. Consider complexity and the expense of computing, it is practicable
...
Usually, $h$ denotes the size of a \textit{window} which is used for computing
local statistics. In the beginning of iterative procedure, the value
is relatively big and only approximates the vicinity of the image curve
due to the high uncertainty. The
...
it makes no sense from the perspective of statistics and can not
capture all features near the contour. Let's denote the
...
Note the goal of the algorithm is to assign each pixel
($\mathrm{v}_{k,l}, k \in [0,\ldots,N_{\mathrm{C}}-1], l \in [0,
2L-1]$) on either side of the contour, it becomes a classical
classification problem. Although each pixel should be assigned to one and only
...
\begin{figure}[htbp]
  ...
  \caption[Distance between a curve point and a pixel in the vicinity
  of a contour]{Distance between a curve point and a pixel in the
...
evaluated offline. Whatever, now the variable
$\frac{d_{k,l}}{\sigma}$ is the linear function with respect to $\mathbf{\Phi}$.
...
we need transform the linear function of $\mathbf{\Phi}$ using a
non-linear activation function $f(\cdot)$~\cite{bishop2006pattern} so that 
\begin{equation}
  \label{eq:nonla}
  a_{v,1} = f(\frac{d_{k,l}}{\sigma})
\end{equation}
Currently, two main non-linear activation functions are popularly used
to solve the classification problem. The first is known as the
\textit{logistic sigmoid} function, and given by
\begin{equation}
  \label{eq:logistic}
...
In this thesis, we use both the logistic sigmoid
function and probit function to implement the algorithm. They give
similar results but have different features. Logistic sigmoid function
...
family, to a non-linear posterior class probabilities. However, not all choices of
class-conditional density are so simple, the probit function is explored
to cope with other complex distributions. The result of logistic
sigmoid function and probit function tend to similar, except the
probit function is sensitive to outliers because the logistic sigmoid
decay asymptotically like $\mathrm{exp}(-x)$ for $x \rightarrow \infty$, whereas
for the probit activation function they decay like $\mathrm{exp}(-x^2)$.
...
array. Now we have restricted our analysis region of interest in the
...
Where $\mathbf{I}$ is just the value of a raw RGB pixel, for a
3-channel image, the values of three components
are  between 0 and 255. Thus local mean vectors $\mathbf{m}_{v,s}$  and local covariance matrices
...
rule in~\cite{hanek2004fitting}, but we do not consider it as the
function with respect to $d_{k,l}$
\begin{equation}
  \label{eq:sigmakl}
...
\label{sec:ref}
In this section, a IRLS  process is executed to refine parameters,
which mean model parameters vector and covariance matrices.
...
distribution is that here the former function has not normalized yet.
...
because $d_{k,l}$ is a function with respect to $\mathbf{\Phi}$, where
$a_{v,1}(d_{k,l})$ holds
\begin{equation}
  \label{eq:nablaa}
...
Now we can compute $\mathbf{J}_{\mathbf{\Phi}}$ by
...
and delete rogue data or outliers~\cite{blake1998active}.
