...
\begin{figure}[htb]
  ...
  \caption[B-spline curves for different degrees~\cite{contourpanin2011}]{Red, green and black
    curves respectively represent B-splines for degree 1,2 and 3.}
\label{fig:bspline}
\end{figure}
...
deformations of a base curve (\cite{blake1998active}). From the perspective of mathematics, shape-space
...
\begin{equation}
  \label{eq:4.16}
  \mathbf{C} = \mathbf{A}\mathbf{X}+ \mathbf{P}_0
\end{equation}
...
In this chapter, we consider an important shape space named planar
...
scaling, so that $\mathbf{T}$ and $\mathbf{M}$ together can represent the 6 degree of
